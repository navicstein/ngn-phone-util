import check from '../src/mobile-detect'
import validate from '../src/validate'
import { networks } from '../src/misc'
import { Providers } from '../src/mobile-detect'
...
describe('Validate', () => {
  it('Validate yields true with a valid phone', () => {
    expect(validate('0802' + generate(7))).toBe(true)
  })
    ...
    for (let prefix of phonePrefixes) {
      ...
      describe('Validate', () => {
        it(`Should pass "${prefix}" with decrepancy of "${network}" with ${randomNumberFromPrefix}`, () => {
          expect(check(randomNumberFromPrefix)).toBe(Providers[network.toLowerCase()])
        })
      })
    }
  }
